...
func CachedResponse(c Cache, req *http.Request) (resp *http.Response, err error) {
	cachedVal, ok := c.GetStream(cacheKey(req))
	if !ok {
		return
	}
...
func (t *Transport) RoundTrip(req *http.Request) (resp *http.Response, err error) {
	...
	if cacheable && canStore(parseCacheControl(req.Header), parseCacheControl(resp.Header)) {
		...
		// Save the original body values
		cl := resp.ContentLength
		body := resp.Body

		// Null out the body to write only the headers to the preamble buffer
		resp.Body = ioutil.NopCloser(strings.NewReader(""))
		hwriter := bufio.NewWriter(&bheader)
		resp.Write(hwriter)
		hwriter.Flush()
		preamble := bheader.Bytes()

		// Set the the response body
		resp.ContentLength = cl
		resp.Body = DeferredTeeReadCloser(body, deferredWriteCloserCallback(func() io.WriteCloser {
			return t.Cache.SetStream(cacheKey)
		}), preamble)
	} else {
		t.Cache.Delete(cacheKey)
	}
...
// bufferedWriteCloser is a wrapper around bytes.Buffer that calls OnEOF
// handler with a full copy of the content when closed
type bufferedWriteCloser struct {
	// OnClose is called with a copy of the content of R when EOF is reached.
	OnClose func(io.Reader)
...
// The return value n is the length of p if the buffer is open; err is nil as long
// as the buffer hasn't been closed, otherwise err will be io.ErrPipeCloseed.
...
// Closes the buffer and calls r.OnClose with the contents.
func (w *bufferedWriteCloser) Close() error {
	w.closed = true
	w.OnClose(bytes.NewReader(w.buf.Bytes()))
	return nil
}
